...
describe('lint factory', function () {
    ...
    describe('getTsLintConfig()', function () {
        it('should fetch the TSLint configuration from file path', function () {
            var tsConfigFilePath = 'tslint.json';
        ...
        it('should extend configuration with {linterOptions} if provided', function () {
            var tsLintConfigFilePath = 'tslint.json';
            var mockConfig = { rulesDirectory: ['node_modules/@ionic'] };
            spyOn(tslint_1.Configuration, tslint_1.Configuration.loadConfigurationFromPath.name).and.returnValue(mockConfig);
            var config = lint_factory_1.getTsLintConfig(tsLintConfigFilePath, {
                typeCheck: true
            });
        ...
        it('should merge {linterOptions} properly if provided in config only', function () {
            var tsLintConfigFilePath = 'tslint.json';
            var mockConfig = { rulesDirectory: ['node_modules/@ionic'], linterOptions: { exclude: ['src/test.ts'] } };
            spyOn(tslint_1.Configuration, tslint_1.Configuration.loadConfigurationFromPath.name).and.returnValue(mockConfig);
            var config = lint_factory_1.getTsLintConfig(tsLintConfigFilePath);
            expect(config.linterOptions).toEqual({
                exclude: ['src/test.ts']
        ...
        it('should merge {linterOptions} properly if provided in config and parameter', function () {
            var tsLintConfigFilePath = 'tslint.json';
            var mockConfig = { rulesDirectory: ['node_modules/@ionic'], linterOptions: { exclude: ['src/test.ts'] } };
            spyOn(tslint_1.Configuration, tslint_1.Configuration.loadConfigurationFromPath.name).and.returnValue(mockConfig);
            var config = lint_factory_1.getTsLintConfig(tsLintConfigFilePath, {
        ...
        it('should overwrite {linterOptions} provided in config with {linterOptions} parameter', function () {
            var tsLintConfigFilePath = 'tslint.json';
            var mockConfig = { rulesDirectory: ['node_modules/@ionic'], linterOptions: { exclude: ['src/test.ts'], typeCheck: false } };
            spyOn(tslint_1.Configuration, tslint_1.Configuration.loadConfigurationFromPath.name).and.returnValue(mockConfig);
            var config = lint_factory_1.getTsLintConfig(tsLintConfigFilePath, {
                typeCheck: true
            });
